...
# import os
# os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Hides oneDNN messages
...
# # --- HTML Template ---
# HTML_TEMPLATE = """{{HTML content remains unchanged}}"""  # (keep your previous HTML template here)
...
#     if request.method == 'POST':
#         if model is None:
#             return render_template_string(HTML_TEMPLATE, error="Model not loaded. Please check server logs.")
...
#         remedy = None if is_healthy else REMEDY_SUGGESTIONS.get(class_name, REMEDY_SUGGESTIONS['default'])
...
#     load_plant_model()
#     if model is None:
#         print("⚠ Model could not be loaded. The app will show an error on upload.")
